...
public:
	...
	virtual ~PHYSICS_ENTITY( void )
	{
	}

	// ~~

	PHYSICS_ENTITY( const PHYSICS_ENTITY & other ) :
	...
	const PHYSICS_ENTITY & operator=( const PHYSICS_ENTITY & other )
	{
		assert( this != &other );
	...
	const MATH_VECTOR_2D & GetPosition( void ) const
	{
		return Position;
	}
	...
	const MATH_VECTOR_2D & GetForce( void ) const
	{
		return Force;
	}
	...
	const MATH_VECTOR_2D & GetVelocity( void ) const
	{
		return Velocity;
	}
	...
	const MATH_VECTOR_2D & GetOldPosition( void ) const
	{
		return OldPosition;
	}
